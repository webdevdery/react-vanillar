...
import { setChild  } from '../../actions/basic'

import brand from '../../assets/img/Page 17 1.png'
import thunbsup from '../../assets/img/thunbsup.png'
import addChild from '../../assets/img/addChild.png'
...
const ChildDetail = () => {
    ...
    const dob = useSelector(state => state.child_detail.child_dob)
    const age = useSelector(state => state.child_detail.child_age)
    const cost_he_1 = useSelector(state => state.child_detail.cost_he_1)
    const expected_inflation_he_1 = useSelector(state => state.child_detail.expected_inflation_he_1)


    const [child_dob, setChildDob] = useState(dob)
    const [child_age, setChildAge] = useState(age)
    const [child_cost, setChildCost] = useState(cost_he_1)
    const [expected_inflation, setExpectedInflation] = useState(expected_inflation_he_1)

    const handleNextButton = () => {
        if(child_dob === '' || child_dob === null || child_dob === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        if(child_age === '' || child_age === null || child_age === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        if(child_cost === '' || child_cost === null || child_cost === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        if(expected_inflation === '' || expected_inflation === null || expected_inflation === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        dispatch(setChild({
            child_dob: child_dob,
            child_age: child_age,
            cost_he_1: child_cost,
            expected_inflation_he_1: expected_inflation
        }))
        dispatch(increment());
        history.push('/member');
    };
    ...
    return (
        ...
        <form className="show_next show_prev" noValidate onSubmit={e => e.preventDefault()}>
                ...
                <div className="col-12 col-sm-12 col-md-8 offset-md-4">
                    <div className="formField">
                        <div className="col-12">
                            ...
                            <h4 className="text-left mb-2">Child Details</h4>
                            <p className="pcolor mb-5">If you have more than 1 child, click on add more to enter details for another child.</p>
                        </div>      
                        <div className="childDetail">
                            <div className="col-md-10 p-0 d-flex flex-wrap">
                                ...
                                <h5 className="mb-3 childNumber">Child <span className="sequence">1</span> Details</h5>
                                <div className="col-12 col-sm-6 mb-4">
                                    <InputField 
                                        ...
                                        required="required"
                                        feedback="Please provide a valid Birthday"
                                    />
                                </div>
                                <div className="col-12 col-sm-6 mb-4">
                                    <InputField 
                                        ...
                                        name="child_edu"
                                        value={ child_cost }
                                        label="Cost of higher education 1 in today's terms"
                                        placeholder="Cost of Higher education"
                                        onChange={e => setChildCost(e.target.value)}
                                        required="required"
                                        feedback="Please provide a valid Cost"
                                    />
                                </div>
                                <div className="col-12 col-sm-6 mb-4">
                                    <InputField 
                            ...
                            <p className="col-12">
                                <span className="btn-copy">
                                    <img src={addChild} alt="Add" /> Add more child
                                </span>
                            </p>
                        </div>
                    </div>
