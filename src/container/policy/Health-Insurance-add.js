...
import { setHealthInsurance  } from '../../actions/policy'
import brand from '../../assets/img/Page 56, 58 1.png'
...
const AddHealthInsurance = () => {
    ...
    const name = useSelector(state => state.health_detail.name_of_insurer)
    const policy = useSelector(state => state.health_detail.policy_type)
    const sum = useSelector(state => state.health_detail.sum_insured)
    const type = useSelector(state => state.health_detail.individual_or_floater)

    const [name_of_insure, setName] = useState(name)
    const [policy_type, setPolicy] = useState(policy)
    const [sum_insured, setSum] = useState(sum)
    const [individual_or_floater, setIndividual] = useState(type)

    const handleNextButton = () => {
        if (name_of_insure == '' || name_of_insure === null ||name_of_insure === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        dispatch(setHealthInsurance({
            name_of_insure: name_of_insure,
            policy_type: policy_type,
            sum_insured: sum_insured,
            individual_or_floater: individual_or_floater
        }))
        dispatch(increment());
                                    ...
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <InputField 
                                                ...
                                                value={ sum_insured }
                                                label="Sum Insured"
                                                placeholder="some insured"
                                                onChange={ e => setSum(e.target.value) }
                                                required="required"
                                                feedback="Please provide a valid sum insured"
                                            />
                                        </div>
                                    </div>                                                 
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                class="form-control padd5"
                                                name="helath_sum_insured"
                                                value={ individual_or_floater }
                                                label="Individual / Floater"
                                                options={[
                                                    {value: 'Individual', label: 'Individual'},
                                                    {value: 'floater', label: 'floater'},
                                                ]}
                                                onChange={ e => setIndividual(e.target.value)}
                                                placeholder="Are you individual or floater"
                                                required="required"
                                                feedback="Please select a valid type"
                                            />
                                        </div>
                                    </div>                                                 
                                </div>
