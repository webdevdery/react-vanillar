...
import { setMotorInsurance  } from '../../actions/policy'

import brand from '../../assets/img/Page 45, 47, 49 1.png'
import thunbsup from '../../assets/img/thunbsup.png'

const AddMotorInsurance2 = () => {
    ...
    const has_cost_of_consumable_items = useSelector(state => state.motor_insurance.has_cost_of_consumable_items)
    const has_zero_debt = useSelector(state => state.motor_insurance.has_zero_debt)
    const has_return_to_invoice_cover = useSelector(state => state.motor_insurance.has_return_to_invoice_cover)
    const has_engine_gear_box_protector = useSelector(state => state.motor_insurance.has_engine_gear_box_protector)
    const has_key_replacement_cover = useSelector(state => state.motor_insurance.has_key_replacement_cover)
    const has_emergency_assistance = useSelector(state => state.motor_insurance.has_emergency_assistance)
    const has_loss_of_use = useSelector(state => state.motor_insurance.has_loss_of_use)

    const [ consumable_item, setConsumableItem ] = useState(has_cost_of_consumable_items);
    const [ zero_debt, setZeroDebt ] = useState(has_zero_debt);
    const [ invoice_cover, setInvoiceCover ] = useState(has_return_to_invoice_cover);
    const [ protector, setProtector ] = useState(has_engine_gear_box_protector);
    const [ key_cover, setKeyCover ] = useState(has_key_replacement_cover);
    const [ emergency_assistence, setEmergencyAssistence ] = useState(has_emergency_assistance);
    const [ loss, setLoss ] = useState(has_loss_of_use);

    const handleNextButton = () => {
        if(consumable_item === '' || consumable_item === null || consumable_item === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        if(zero_debt === '' || zero_debt === null || zero_debt === undefined) {
            $('form').addClass('was-validated');
            return false;
        }
        dispatch(setMotorInsurance({
            has_cost_of_consumable_items: consumable_item,
            has_zero_debt: zero_debt,
            has_return_to_invoice_cover: invoice_cover,
            has_engine_gear_box_protector: protector,
            has_key_replacement_cover: key_cover,
            has_emergency_assistance: emergency_assistence,
            has_loss_of_use: loss,
        }))
        dispatch(increment());
        history.push('/final');
    };
                        ...
                        <div className="col-12 col-sm-12 col-md-8 offset-md-4">
                            <div className="formField">
                                <div className="col-12">
                                    ...
                                    <h4 className="text-left mb-2">Together with Motor Insurance, have you bought?</h4>
                                </div>
                                    ...
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                value={ zero_debt }
                                                label="Zero Dep"
                                                onChange={e => setZeroDebt(e.target.value)}
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                    {value: '30', label: '30'},
                                                ]}
                                                placeholder="select the Zero Dep"
                                                required="required"
                                    ...
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                onChange={e => setInvoiceCover(e.target.value)}
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                    {value: '30', label: '30'},
                                                ]}
                                                placeholder="select the return to invoice cover"
                                                required="required"
                                                feedback="Please select a valid Invoice Cover"
                                            />
                                        </div>
                                    </div>                                                  
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                value={ consumable_item }
                                                label="Cost of Consumable Items"
                                                onChange={e => setConsumableItem(e.target.value)}
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                    {value: '30', label: '30'},
                                                ]}
                                                placeholder="select the cost of consumable"
                                                required="required"
                                                feedback="Please select a valid Cost of Consumable Items"
                                            />
                                        </div>
                                    </div>                                                                                               
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                placeholder="Select the value of NCB Protection"
                                                required="required"
                                                feedback="Please select a valid NCB Protection"
                                            />
                                        </div>
                                    </div>                                                     
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                class="form-control padd5"
                                                name="key_replacement_cover"
                                                value={ key_cover }
                                                label="Key Replacement Cover"
                                                onChange={ e => setKeyCover(e.target.value) }
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                    {value: '30', label: '30'},
                                                ]}
                                                placeholder="select the key replacement cover"
                                                required="required"
                                                feedback="Please select a valid Key Replacement Cover"
                                            />
                                        </div>
                                    </div>                                               
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                value={ protector }
                                                label="Engine & Gear Box Protector"
                                                onChange={e => setProtector(e.target.value)}
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                ...
                                                placeholder="Select the Engine & Gear box protector"
                                                required="required"
                                                feedback="Please select a valid Engine & Gear Box Protector"
                                            />
                                        </div>
                                    </div> 
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                ...
                                                value={ emergency_assistence }
                                                label="Emergency Assistance"
                                                onChange={e => setEmergencyAssistence(e.target.value)}
                                                options={[
                                                    {value: '10', label: '10'},
                                                    {value: '20', label: '20'},
                                                    {value: '30', label: '30'},
                                                ]}
                                                placeholder="Select the Emergency Assistance"
                                                required="required"
                                                feedback="Please select a valid Emergency Assistance"
                                            />
                                        </div>
                                    </div> 
                                    <div className="col-md-6">
                                        <div className="inputField my-4">
                                            <SelectField 
                                                class="form-control padd5"
                                                name="downtime_protection"
                                                value={ loss }
                                                label="Loss Of Use - Downtime Protection"
                                                ...
                                                placeholder="Select the downtime protection"
                                                required="required"
                                                feedback="Please select a valid Downtime Protection"
                                            />
                                        </div>
                                    </div> 
                                </div>
...
AddMotorInsurance2.propTypes = {
  consumable_item: PropTypes.string,
  zero_debt: PropTypes.string,
  invoice_cover: PropTypes.string,
  protector: PropTypes.string,
  key_cover: PropTypes.string,
  emergency_assistence: PropTypes.string,
  loss: PropTypes.string
};

export default AddMotorInsurance2;
